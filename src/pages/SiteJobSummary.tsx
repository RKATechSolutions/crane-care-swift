...
export default function SiteJobSummary({ onCreateQuote, activeJobId, isRemoteSignoff }: SiteJobSummaryProps) {
  ...
  const [pendingIncludeLiftingRegister, setPendingIncludeLiftingRegister] = useState(true);
  ...
  const applyReportSelection = () => {
    ...
    setIncludeLiftingRegister(pendingIncludeLiftingRegister);
        ...
        {showReportSelector && (
          <div className="fixed inset-0 z-[110] bg-background flex flex-col">
            ...
            <div className="flex-1 overflow-auto">
              ...
              <label className={`flex items-center gap-3 px-4 py-4 border-b border-border cursor-pointer transition-colors ${pendingIncludeLiftingRegister ? 'bg-primary/5' : ''}`}>
                <input
                  type="checkbox"
                  checked={pendingIncludeLiftingRegister}
                  onChange={e => setPendingIncludeLiftingRegister(e.target.checked)}
                  className="h-4 w-4 rounded accent-primary flex-shrink-0"
                />
                <div>
                  <p className="font-medium text-sm">Lifting Equipment Register defects</p>
                  <p className="text-[11px] text-muted-foreground">Include failed / removed-from-service items for customer review</p>
                </div>
              </label>
              {/* Inspection reports */}
            ...
            <div className="p-4 border-t border-border bg-background">
              <p className="text-xs text-muted-foreground text-center mb-3">
                {pendingSelectedIds.size} report{pendingSelectedIds.size !== 1 ? 's' : ''} selected{pendingIncludeLiftingRegister ? ' • lifting register defects included' : ''}
              </p>
              <button
                onClick={applyReportSelection}
                disabled={pendingSelectedIds.size === 0 && !pendingIncludeLiftingRegister}
          ...
          <div className="flex items-center justify-between mb-2">
            ...
            <button
              onClick={() => {
                ...
                setPendingIncludeLiftingRegister(includeLiftingRegister);
            ...
            {defectsExpanded && !dbDefectsLoading && (
              <>
                ...
                {hasDbDefects && (() => {
                            ...
                            <div className="flex flex-wrap gap-1.5 mt-1.5">
                              {defect.quoteStatus && (
                                <span className={`text-[10px] font-bold px-2 py-0.5 rounded ${
                                  defect.quoteStatus === 'Quote Now' ? 'bg-rka-green text-primary-foreground' : 'bg-foreground text-background'
                                }`}>{defect.quoteStatus === 'Quote Now' ? '✓ Fix Now' : '⏳ Quote Later'}</span>
                              )}
                              {defect.urgency && (
                  ...
                  return (
                    <>
                      {uncategorizedDb.length > 0 && (
                        <div className="mb-4">
                          <p className="text-xs font-bold text-muted-foreground uppercase tracking-wide mb-2 flex items-center gap-1.5">
                            <AlertTriangle className="w-3.5 h-3.5" /> Needs Customer Decision ({uncategorizedDb.length})
                          </p>
                          {uncategorizedDb.map(renderDefectCard)}
                        </div>
                      )}
                      {quoteNowDb.length > 0 && (
                        <div className="mb-4">
                          <div className="flex items-center gap-2 mb-2 p-2 rounded-lg bg-rka-green-light">
                            <Check className="w-4 h-4 text-rka-green" />
                            <p className="text-xs font-bold text-rka-green-dark uppercase tracking-wide">Fix Now — Quote Required ({quoteNowDb.length})</p>
                          </div>
                          {quoteNowDb.map(renderDefectCard)}
                        </div>
                      )}
                      {quoteLaterDb.length > 0 && (
                        <div className="mb-4">
                          <div className="flex items-center gap-2 mb-2 p-2 rounded-lg bg-muted">
                            <FileText className="w-4 h-4 text-muted-foreground" />
                            <p className="text-xs font-bold text-muted-foreground uppercase tracking-wide">Quote Later ({quoteLaterDb.length})</p>
                          </div>
                          {quoteLaterDb.map(renderDefectCard)}
                        </div>
                      )}
                    </>
                  );
